	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		if ( have_posts() ) {
			...
			<?php
				...
				<?php if('on' === $show_date){?>
				<div class="press_title playfair-display">
						Press  /  <?php echo esc_html( get_the_date( $meta_date ) );?>
					</div>
				<?php } ?>
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				<?php if ( ! in_array( $post_format, array( 'link', 'audio' ) ) || post_password_required( $post ) ) { ?>
					<<?php echo $processed_header_level; ?> class="entry-title">
				...
				if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_categories || 'on' === $show_comments ) {
						printf( '<p align="center" class="post-meta">%1$s %2$s %3$s %4$s %5$s %6$s %7$s</p>',
						//printf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s %6$s </p>',
							(
								'on' === $show_author
									? et_get_safe_localization( sprintf( __( 'by %s', 'et_builder' ), '<span class="playfair-display ">' .  et_pb_get_the_author_posts_link() . '</span>' ) )
									: ''
							),
							(
				...
				<?php echo '<div class="post-content">';
